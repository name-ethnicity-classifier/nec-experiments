...
class TripleNGramLSTM(nn.Module):
    def __init__(self, class_amount: int=0, hidden_size: int=10, layers: int=2, dropout_chance: float=0.5, bidirectional: bool=False, embedding_size: int=64):
        ...
        self.attention1 = nn.MultiheadAttention(self.hidden_size * self.directions, num_heads=2, dropout=0.15)
        self.attention2 = nn.MultiheadAttention(self.hidden_size * self.directions, num_heads=2, dropout=0.15)
        self.attention3 = nn.MultiheadAttention(self.hidden_size * self.directions, num_heads=2, dropout=0.15)
