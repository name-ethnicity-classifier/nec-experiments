...
class NameEthnicityDataset(torch.utils.data.Dataset):
    def __init__(self, dataset: list=[], class_amount: int=10, augmentation: bool=False, n_gram: List[int]=[1]):
        """ constructor
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        if len(self.n_gram) == 1:
            int_name_n1 = int_name
            return torch.Tensor(int_name_n1), torch.Tensor(target).type(torch.LongTensor), non_padded_sample
