...
def custom_collate(batch):
    ...
    sample_n1_batch, target_batch, non_padded_batch = [], [], []
    for sample_n1, target, non_padded_sample in batch:
    ...
    return padded_n1_batch, torch.cat(target_batch, dim=0).reshape(len(sample_n1_batch), target_batch[0].size(0)), non_padded_batch


def create_dataloader(dataset_path: str="", test_size: float=0.01, val_size: float=0.01, batch_size: int=32, class_amount: int=10, \
                                                                            augmentation: bool=False, n_gram: List[int]=[1]):
